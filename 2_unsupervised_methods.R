...
# this creates a paragraph to vec model: 
# PV-DBOW: uses the PV Bag of Words Model
# dim: decide on number of observations of a single word
# iter: how many iterations should be done
...
# the dimensionality of the word embeddings data is given by the vocabularity and the chosen dimensionality of a single word
dim(word_embeddings)
...
# as can be seen the context of decline and increase is much more similar than that between decline and canada
...
# plot the first two principal components of document embeddings
# separately pre and post 2008. 
# This allows one to compare whether the words used changed between both periods. 
# more specifically, it shows whether words with different meanings are used. It would
# not show if different words with the same meaning are used, e.g. "hi" instead of "hello"
